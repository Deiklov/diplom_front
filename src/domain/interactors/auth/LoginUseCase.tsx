...
export default class LoginUseCase {
    ...
    public async registerUser(email: string, password: string, name: string, age: number, login: string): Promise<void> {
        // const validationResult = await this.authRepository.validateCredentials(email, password);
        const authResult = await this.authRepository.register(email, password, name, age, login);
